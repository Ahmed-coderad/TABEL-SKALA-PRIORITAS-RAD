...
function renderTasks(){
  ...
  tasks.forEach(task => {
    ...
    const reminderBadge = task.reminderAt
      ? `<small>🔔 ${new Date(task.reminderAt).toLocaleString(getLocaleTag())}</small>`
      : '';
    ...
    card.innerHTML = `
      ...
      <div class="task-footer">
        ...
        <button class="done-btn">
          ${task.completed ? t('taskUndo') : t('taskDone')}
        </button>
  ...
  ['q1','q2','q3','q4'].forEach(id => {
    ...
    if(container.innerHTML.trim() === ''){
      container.innerHTML =
        `<div class="empty">${t('emptyQuadrant')}</div>`;
    }
